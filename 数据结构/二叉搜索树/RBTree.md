...
双红修正看 (插入点)$x$ 的 $uncle$ 颜色，牵涉到的节点为插入节点 $x$，以及 $x$ 的 $p, g, u$ 。
...
这等价于 **1次connect34 + 2次染色**（$g$一定由黑边红，因为其一定当不了 $b$ ，然后当 $b$ 的那个节点由红变黑）。
...
* $u$ 为黑 $\Rightarrow$ $RR-1$ ，**1次connect34 + 2次染色**。调整完毕。
...
双黑修正要找处理位置 `x/r`，先看`x`的兄弟`s`(sibling，也是 r 的 uncle)，然后看`s`的儿子`t`，然后看`s`的父亲`p`。

* 若`s`为黑，则为 $BB-1 / BB-2R / BB-2B$
    + 若`s`有红孩子`t`，则为 $BB-1$
