...
/* 与 p435_impl3.cpp 的写法对比，
把 query 和 加点 收集混合到一起，然后靠 type 保证先加点再查询，类似 洛谷/p1972_impl3.cpp 的写法
*/
...
struct Op { int type, l, r, id; } op[3 * maxn]; int total;
...
int main(int argc, char const *argv[]) {
    ...
    sort(op + 1, op + 1 + total, cmp);
    for (int i = 1; i <= total; i++) { // 扫描线从左到右扫的是 op 的下标，或者说 op.r ，而 op.r 就是 nums 的下标，也是相当于 query 被挂到了 r 位置
        // 加点
        auto [type, l, r, id] = op[i];
        if (type == 0) {
