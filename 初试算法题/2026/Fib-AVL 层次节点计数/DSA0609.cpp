// https://www.smqyoj.com/p/DSA0609
// https://www.smqyoj.com/record/694e60729ba766269a4fd081
...
namespace my {
    ...
    namespace io {
        ...
        void print(const std::string& fmt, Args&&... args) {
            print_fmt_impl(fmt.begin(), fmt.end(), std::forward<Args>(args)...);
        }
...
int calc(int h, int k) {
    ...
    for (int i = 2; i <= h; i++) {
        f[i % 3][0] = 1;
        for (int j = 1; j <= i; j++) {
            f[i % 3][j] = ((long long)f[(i - 2 + 3) % 3][j - 1] + f[(i - 1 + 3) % 3][j - 1]) % MOD;
        }
    }
    return f[h % 3][k];
}
